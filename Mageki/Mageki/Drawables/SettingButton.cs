...
namespace Mageki.Drawables
{
    public class SettingButton : ButtonBase
    {
        ...
        {
            using var fontStream = GetType()
                .Assembly.GetManifestResourceStream(
                    "Mageki.Assets.Fonts.MaterialIconsRound-Regular.otf"
                );
            ...
            // NFC is only initialized when the platform-specific Essentials implementation is available.
            try
            {
                if (DeviceInfo.Platform == DevicePlatform.Android)
                {
                    DependencyService
                        .Get<INfcService>()
                        .StartReadAime(ScanFelica, ScanMifare, () => { });
                }
            }
            catch (Exception ex)
            {
        ...
        public override void Draw(SKCanvas canvas)
        {
            ...
            var fontRect = SKRect.Inflate(
                boundingBox,
                boundingBox.Height * -0.1f,
                boundingBox.Height * -0.1f
            );
            ...
            if (TouchCount == 0)
            {
                ...
                canvas.DrawText(
                    icon.ToString(),
                    new SKPoint(fontRect.MidX, fontRect.Bottom),
                    iconPaint
                );
            }
            else
        ...
        public override bool HandleTouchPressed(long id, SKPoint point)
        {
            if (!(nfcScanning || logoHoldUnhandled))
            {
                ...
                if (
                    ...
                    || !DependencyService.Get<INfcService>().ReadingAvailable
                )
                {
                    ...
                    timer.Elapsed += (sender, e) =>
                    {
                        MainThread.InvokeOnMainThreadAsync(() =>
                        {
                            if (logoHoldUnhandled)
                            {
                                ...
                                nfcService.StartReadAime(
                                    ScanFelica,
                                    ScanMifare,
                                    ScanFelicaInvalidated
                                );
                                timer.Dispose();
                            }
                        });
        ...
        public async void ScanFelica(byte[] packet)
        {
            ...
            string idmString =
                "0x" + BitConverter.ToUInt64(packet.Skip(0).Take(8).ToArray(), 0).ToString("X16");
            string pmMString =
                "0x" + BitConverter.ToUInt64(packet.Skip(8).Take(8).ToArray(), 0).ToString("X16");
            string systemCodeString = BitConverter
                .ToUInt16(packet.Skip(16).Take(2).ToArray(), 0)
                .ToString("X4");
            App.Logger.Debug(
