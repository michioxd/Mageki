...
namespace Mageki.Drawables
{
    public class Lever : Box
    {
        ...
        public override void Update()
        {
            ...
            // 摇杆末端在最边上的高度
            var endPointMinHeightMM = stickLengthMM * (float)Math.Cos(Math.PI / 6);
            // 计算面板平面上像素密度
            var panelPlanePixelPerMM =
                endPointRangePx
                / endPointRangeMM
                / eyePanelDistanceMM
            ...
            holePath.ArcTo(
                new SKRect(holeRight, holeTop, holeRight + holeHeightPx, holeBottom),
                90,
                -180,
                false
            );
            ...
            holePath.ArcTo(
                new SKRect(holeLeft - holeHeightPx, holeTop, holeLeft, holeBottom),
                -90,
                -180,
                false
            );
            ...
            // 摇杆末端高度
            var endPointHeightMM = stickLengthMM * (float)Math.Cos(Math.PI / 6f * Value);
            // 摇杆末端水平面的像素密度
            var endPointPlanePixelPerMM =
                panelPlanePixelPerMM
                / (eyePanelDistanceMM - (endPointHeightMM - panelShaftDistanceMM))
                * eyePanelDistanceMM;
            ...
            var limiterPlaneHeightMM =
                -panelShaftDistanceMM + limiterRangeMM / 2 / (float)Math.Tan(Math.PI / 6f);
            var limiterPlanePixelPerMM =
                panelPlanePixelPerMM
                / (eyePanelDistanceMM - (limiterPlaneHeightMM - panelShaftDistanceMM))
                * eyePanelDistanceMM;
            ...
            // 摇杆当前角度下切面经过透视的宽高比
            var ratio = (float)Math.Cos(Math.PI / 3 * Value);
            // 摇杆末端，会被摇杆头遮住，无需绘制
            //stickTop.Reset();
            ...
            stickSide.Reset();
            stickSide.MoveTo(
                holeMidX + endPointRangePx / 2f * Value,
                holeMidY + endPointStickDiameterPx / 2f
            );
            stickSide.LineTo(
            ...
            stickSide.ArcTo(
                new SKRect(
                    holeMidX
                        + limiterRangePx / 2f * Value
                        - limiterPlaneStickDiameterPx / 2 * ratio,
                    holeMidY - limiterPlaneStickDiameterPx / 2f,
                    holeMidX
                        + limiterRangePx / 2f * Value
                        + limiterPlaneStickDiameterPx / 2 * ratio,
            ...
            stickSideBorderPaint.Shader = SKShader.CreateLinearGradient(
                ...
                new SKPoint(
                    holeMidX
                        + limiterRangePx / 2f * Value
                        + (limiterPlaneStickDiameterPx / 2f * ratio * (Value < 0 ? 1 : -1)),
                    holeMidY
                ),
                new SKColor[]
                {
                    stickSideBorderPaint.Color,
                    stickSideBorderPaint.Color.WithAlpha(0),
                },
            ...
            stickHighLightPath.MoveTo(
                holeMidX + limiterRangePx / 2f * Value,
                holeMidY + limiterPlaneStickDiameterPx * -0.1f / 2f
            );
            stickHighLightPath.LineTo(
                holeMidX + endPointRangePx / 2f * Value,
            ...
            // 摇杆帽底面相对于转轴的高度
            var capBottomHeightMM =
                (stickLengthMM - capHeightMM) * (float)Math.Cos(Math.PI / 6f * Value);
            var capBottomPlanePixelPerMM =
                panelPlanePixelPerMM
                / (eyePanelDistanceMM - (capBottomHeightMM - panelShaftDistanceMM))
                * eyePanelDistanceMM;
            ...
            capSidePath.LineTo(
                holeMidX + capBottomRangePx / 2f * Value,
        ...
        public override void Draw(SKCanvas canvas)
        {
            if (!Visible)
                return;
            backPaint.Color = CanvasPalette.LeverBack;
            holePaint.Color = CanvasPalette.LeverHole;
            stickSidePaint.Color = CanvasPalette.LeverSide;
            stickSideBorderPaint.Color = CanvasPalette.LeverBorder;
            stickHighLightPaint.Color = CanvasPalette.LeverHighlight;
            capPaint.Color = CanvasPalette.LeverBorder;
            capStrokePaint.Color = CanvasPalette.Medium;
            base.Draw(canvas);
        ...
        {
            if (
                (Settings.LeverMoveMode == LeverMoveMode.Absolute && touchPoints.Count == 0)
        ...
        public override void HandleTouchReleased(long id)
        {
            ...
            if (Settings.EnableLeverOverflowHandling && touchPoints.Count == 0)
                valueOverflow = 0;
        }
    }
}
